...
#define MOTION_DRIVER_TARGET_MSP430    //�����������֣�����MSP430��������ֲ��STM32F1��
...
int mpu_init(void)
{
    ...
    if (rev) {
        ...
        if (rev == 1)
            ...
            /* 兼容未知 rev 的模块，不再直接失败 */
            log_e("Unknown product rev %d, use full sensitivity.\n", rev);
            st.chip_cfg.accel_half = 0;
        }
    } else {
        ...
        rev = data[0] & 0x0F;
        if (!rev) {
            /* 大量国产/兼容 MPU6050 的 prod_id 读到 0，原厂代码会直接失败。
             * I2C 已通时按正常灵敏度继续初始化，避免 MPU FAIL。 */
            log_e("Product ID=0 (clone?). Continue with full sensitivity.\n");
            st.chip_cfg.accel_half = 0;
        } else if (rev == 4) {
